...
- **matugen and the Z13 template**, see below.
...
## matugen and the Z13 rear-window color
...
`chezmoi/dot_config/matugen/config.toml` declares one user template,
`z13_window`:
...
Any wallpaper or theme change re-renders it. Trigger one explicitly with:
...
cache file was written and matches what the rear window should show:
